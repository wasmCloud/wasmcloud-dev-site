...
Run `make` to build the actor and re-sign it. If there are no errors compiling the actor, all existing instances of the `hello` actor will be scaled down and replaced with instances of the updated actor thanks to the hot reload configuration that you did earlier.
