...
We know our new actor needs a web server, so let's start the HTTP server capability provider. To do that, click the **Start Provider** button and enter this OCI URL: `wasmcloud.azurecr.io/httpserver:0.16.0`. Now we should have both an actor and a provider, and the dashboard should look like this:
